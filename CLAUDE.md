...
**단일 소스(SSOT): `agawaeuleo_spec.md` (v1.3.1)** — 모든 수치·문구·동작의 계약. 단, **시각 레이어(색·서체·일러스트·컴포넌트 외형)는 `agawaeuleo_design_clay.md`(DESIGN v3 "몽글 클레이")가 우선**, **모션 계약은 `agawaeuleo_design_premium.md`(DESIGN v2)가 계속 유효**(스펙 §16.4 v1.3.1 참조). 구현과 스펙이 어긋나면 스펙을 먼저 확인하고, 스펙 내부 모순이면 §3.3(게스트 우선) 원칙이 우선.

## 현재 상태 (2026-09-24 기준)

- **디자인 v3 "몽글 클레이" 전면 개편(2026-09-24, 발주자 요청 — "초보맘을 위한 여성스럽고 아기자기한 일러스트 + UI/UX, 레퍼런스 3D 캐릭터 스타일로 일러스트 전부 교체, 애니메이션은 유지")**: 계약은 `agawaeuleo_design_clay.md`. 요지 —
  - **토큰 값만 교체, 이름은 유지**(`paper*`=표면, `ink*`=텍스트, `seal`=브랜드 딸기 핑크): 딸기우유 크림 `#FFF6EF` + 코코아 텍스트 + 로즈 `accent #B53E5D`(글자용) + **`accentFill #D9557A`(채운 알약 면 전용 — 위 글자는 반드시 주아체 19 이상 = WCAG 큰 텍스트)** + 파스텔 워시 + 엄마 돌봄 `lilac/lilacWash`(신규). wash 위 fg 글자 4.6~4.8:1 실측. 모서리 한 단계 상향(xs8·sm14·md20·lg26·xl34), 장밋빛 음영, **클레이 광택(세로 2색) 그라데이션만 허용**(`widgets/surfaces/clay_sheen.dart` — `ClaySheen.gradient/toneShadow/bubble` + `StickerChip`).
  - **서체**: 명조(NotoSerifKR 8.5MB) 제거 → 제목 `display/title/heading` = **주아체 Jua(OFL, 한 굵기 — `fontWeight` 바꾸지 말 것, 가운뎃점은 `AppFontFamily.displayFallback`의 Pretendard가 받음)**, 본문 Pretendard 유지, 숫자 강조는 모노 대신 주아체.
  - 작은 글자가 필요한 알약(장 탭·세그먼트)은 채운 톤 면 대신 **크림 젤리 알약 + 톤 글자**. 먹 은유(먹선 틱·붓결·모노 번호) 걷어냄. 모든 모션은 v2 그대로.
  - 신규 공용: `ClaySceneArt`(states/), `EmptyState/ErrorState.illustrationAsset`, `PrimaryButton.tone`, `AppCard.stackColor`, `PaperBlobPainter.planetShapes`, 홈 `DeckPalette`, 상세 `detail_clay.dart`(`DetailTone`/`ClayBubble`/`ClayNumber`), 온보딩 `orbit_charms_painter.dart`. 스플래시는 두 얼굴 에셋을 미리 올릴 때까지(최대 300ms) 첫 프레임을 보류한다(`deferFirstFrame`).
  - 검증: `flutter analyze` 0 · 테스트 216 통과 · iOS 시뮬레이터 라이트/다크 실화면(스플래시·온보딩 3장·홈·상세·내 정보·설정·검색) 확인.
...
- 시그니처 컴포넌트(전부 `lib/presentation/widgets/`, **이름은 v2 그대로·외형은 v3**): `InkSeal`(딸기 스티커 도장)·`InkHaloIcon`(클레이 버블)·`BrushDivider`(몽글 점선)·`SectionHeader`(액센트 점 + 주아체)·그레인 `PaperBackground`·`AppSheetShell`/`AppDialogShell`·`SlidingSegment`(크림 젤리 썸), `AppCard` emphasis 3단(flat/raised/hero — hero 아랫장 파스텔), 증상 카테고리 톤 `SymptomTone`, `StickerChip`/`ClaySheen`. 새 UI는 이 컴포넌트들을 우선 재사용할 것.
- **클레이 일러스트**(v3 — 증상 32종 + 장면 9종: 온보딩 3·스플래시 얼굴 2·빈 상태 3·내 정보 1, 768² WebP 투명 + 바닥 그림자, 총 ≈3.3MB): **수정은 반드시 `tool/illustrations/clay/`에서** — 엔진 `clay.py`(numpy 2.5D 높이맵 점토 렌더러: 2D SDF 윤곽 + 둥근 단면 → 좌상단 키라이트·AO·자기 그림자·점토 산란·광택 + 우하단 바닥 그림자), 부품 `characters.py`(주인공 아가 — 호빵 머리·**물음표처럼 말린 배냇머리 한 가닥**·점 눈·볼터치·표정별 입 / 엄마 / 몸통 / 공용 소품)·`props.py`(증상별 소품), 장면 `generate_clay_illustrations.py`. 실행: `python3 tool/illustrations/clay/generate_clay_illustrations.py`(전부 + `symptom_illustration_data.dart` 키 목록 생성) / `--only fever,burp --sheet /tmp/s.png --px 384`(미리보기 시트, 에셋 안 씀). numpy+Pillow만 필요. 에셋·생성 Dart 직접 편집 금지. 위젯은 `SymptomIllustration(illustrationKey:, size:)`·`ClayIllustration(asset:, size:)`·경로 상수 `ClayScenes`(120dp 이하는 384px로 디코드). 옛 벡터 엔진(`InkIllustration`/`IllustrationShape`)과 생성기는 삭제됨. 카드 한 줄 설명은 `symptoms.tagline`(공백 포함 8자 이내, 픽스처와 동일 값 유지).
...
- **하단 네비 재설계 + 앱 아이콘·이름**: (2026-09-11 **먹 캡슐**로 다시 교체 — 가운데 작은 캡슐 + 먹이 번지듯 흐르는 알약 + 알약 가장자리에서 글자가 뒤집히는 두 겹 라벨, 두 탭 라벨 상시 노출. v3부터 알약은 `accentFill`(+클레이 광택)·라벨 주아체 19 — 계약은 DESIGN v3 §5.3, 모션은 v2 §5.3. 아래는 직전 독 설계 기록) `app_bottom_nav.dart`를 **플로팅 페이퍼 독 + 모핑 잉크 알약**으로 재설계 — 좌우 x16·하단 x12 부양 독(paperRaised·line 전체 보더·e4·스타디움), 활성 탭만 `accentWash` 알약이 아이콘+라벨로 펼쳐지는 모핑(spring, accent 헤어라인 α0.22), 비활성은 아이콘 단독(ink500)+`Semantics` 라벨 보강, 햅틱·reduce-motion·textScaler ≤1.3+FittedBox 방어 유지(테스트 2종은 아이콘 탭 기준으로 갱신됨). 런처 표기명 '아가왜울어'(AOS `values/strings.xml` app_name + manifest label, iOS `CFBundleDisplayName`). 앱 아이콘은 (v3) **딸기 핑크 `#F28DA2` 바탕 + 클레이 우는 아가 얼굴**(일러스트 엔진의 `sc_baby_cry` 그대로 — 왼뺨 눈물 한 방울) — **수정은 반드시 `tool/app_icon/generate_app_icon.py`에서** 재생성(생성 PNG 직접 편집 금지, 얼굴은 `tool/illustrations/clay/`를 임포트). AOS 적응형(`mipmap-anydpi-v26/ic_launcher.xml` + `values/colors.xml` #F28DA2 + 클레이 fg / 테마 아이콘 mono는 v2 벡터 마크 유지)·레거시 5밀도, iOS 15종(풀블리드·알파 없음), 스토어 512.
...
- **온보딩 전면 개편(DESIGN v2 §7.6-6 v2.2 "히어로 스테이지", 2026-09-11)**: 아이콘 프레임 3장 → 우는 아기/케어 가방/구름 위 잠 일러스트(v3부터 클레이 `ClayScenes.onboardingCry/Care/Sleep`) + 실제 UI 조각(검색 타이핑·BEST PICK·병원 신호 카드 — 3장은 2026-09-11 기록 숨김으로 교체)을 **페이지뷰 밖 고정 스테이지**에 올려 페이지 값에 1:1 연동. 옛 느린 패럴랙스(옆 그림이 스와이프 첫 프레임에 불쑥 보이던 원인)와 마지막 장 `InkSeal`은 제거. 제목은 카피에서 줄을 직접 나누고 본문은 `core/utils/keep_all.dart`(어절 단위 줄바꿈). 하프톤 도트는 `drawPoints` 한 번으로 그려 홈 카드도 가벼워짐. 검증은 `test/features/onboarding_screen_test.dart`.
...
- **스플래시 개편(2026-09-11 → v3)**: 네이티브 런치 화면(iOS 스토리보드·안드로이드 런치 테마, 라이트/다크 — 크림 `#FFF6EF`/코코아 `#221A19`)을 **딸기 핑크 원 + 클레이 우는 얼굴 배지**로 두고, Flutter 스플래시가 **같은 배지에서 이어받아** 우는 얼굴(`baby_cry.webp`) → 방긋(`baby_smile.webp`) 크로스페이드 + 스쿼시 바운스, 오른뺨 벡터 눈물이 흘러내림, 블롭·점선 궤도·하트/별 참 + 주아체 워드마크. 배지 에셋은 **`tool/app_icon/generate_app_icon.py --splash`에서만** 생성(원 지름 대비 얼굴 박스 0.78 = `SplashMark.markRatio`, 두 얼굴 WebP는 픽셀 정렬 동일 — 클레이 생성기에서 함께 재생성). 부팅 최소 노출 600→**1150ms**(연출 길이와 동일, 테스트 하네스 대기 1300ms). DESIGN §7.6-1.
- 검증 그린: `flutter analyze` 0 이슈, 테스트 전체 통과(2026-09-24 기준 216개), `flutter build apk --debug` ✓, iOS 시뮬레이터 실행 ✓(제품 섹션 개편 후 재검증 — 실데이터/실이미지 렌더 확인).
...
- 색/크기/모션 하드코딩 금지 — `lib/config/theme/` 토큰만 (토큰 값의 계약은 DESIGN v3 §3 — 임의 변경 금지, 대비 실측값 유지). reduce-motion은 AppMotion 리졸버 경유.
...
- `agawaeuleo_design_clay.md` — **DESIGN v3 "몽글 클레이"** 시각 레이어 계약 (토큰·클레이 일러스트 파이프라인·컴포넌트·화면별 지시)
- `agawaeuleo_design_premium.md` — DESIGN v2 (시각 레이어는 v3로 대체, **모션 계약은 유효**)
