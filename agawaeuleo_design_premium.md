...
6. **온보딩** (v2.2 개정 — "히어로 스테이지", 2026-09-11. 아이콘-in-프레임·스텝 라벨·마지막 장 낙관은 폐기):
   - **서사**: 우는 아기(증상 검색) → 케어 가방(맞춤 용품) → 구름 위 잠(기록). 일러스트 3종은 증상 일러스트와 같은 엔진·그림체(`InkIllustration`, 뷰박스 **240** — 선 수치는 그대로 두고 뷰박스만 키워 히어로에서도 먹선 굵기를 카드와 맞춤). **수정은 `tool/illustrations/generate_onboarding_illustrations.py --dart`에서만**(생성 파일 직접 편집 금지).
   ...
   - **스테이지는 페이지뷰 밖 고정 무대**: 세 장면을 항상 올려 두고 페이지 값에 1:1로 레이어별 이동·페이드(일러스트 110 / 뒤 카드 150 / 앞 카드 230). 옛 "느린 패럴랙스"는 옆 페이지 그림을 화면 안으로 끌어와 스와이프 첫 프레임에 불쑥 보이는 버그의 원인이었으므로 **금지**. 일러스트는 중간 지점 전에 사라져(fade 2.1) 선화 두 장이 반투명으로 겹쳐 탁해지지 않게 한다. 공유 배경은 종이 오림 블롭(`paperStack` 아랫장) — 장면 워시(accent→amber→sage)와 윤곽이 모핑, 점선 궤도·행성 3개(카드와 겹치지 않는 각도).
   - **글**: 페이지와 함께 움직이되 오버라인 24 / 제목 48 / 본문 72만큼 더 빨리 나감(항상 화면 바깥 방향) + 페이드.
   - **마이크로 인터랙션**(스크롤이 **멈춘** 뒤 재생 — `onPageChanged`는 0.5 지점이라 쓰지 않음): 검색어 한글 IME 자모 타이핑 + 캐럿, 1위 배지 스탬프, 수면 타이머 1초 틱 + 숨쉬는 점, "방금" 기록 행 내려앉기. 첫 진입 1회 등장(배경→일러스트→카드, 900ms), 카드 호흡 ±3dp 상시. reduce-motion이면 전부 완료 상태 정지.
