...
1. **구성(위→아래)**: 인사(날짜 overline + 명조 인사 + 오늘의 응원 + 메뉴 — 현행) → 검색 알약 → **케어 덱**(그룹 탭 + 무대 + 글 + 레일). 덱이 남은 높이를 모두 쓰는 한 화면 구성, 작은 화면은 덱 최소 높이(470)를 지키며 세로 스크롤. 당겨서 새로고침(잉크 드롭) 유지. 글자 배율 상한 1.3.
...
4. **무대**(캔버스 360×300, contain): `PaperBlobPainter`(증상 `SymptomTone` 워시로 모핑, 중심 180,150·반경 112·궤도 1.17) + 일러스트 212(`holdInkWeight`, 블롭 중심에서 왼쪽 위로 18·12 비킴) + 떠 있는 **실데이터 조각**(e4): 좌상 칩 "추천 용품 N"(용품 없으면 "병원 신호 N") · 우하 **BEST PICK 카드**(1위 제품 제목 2줄, 폭 172). BEST 카드 탭 → 상세 '추천 용품' 장으로 바로(외부 링크는 대가성 표시가 있는 상세에서만 — §13.2). 조각 데이터는 상세와 같은 프로바이더를 공유(덱에서 본 증상은 상세 진입 시 이미 캐시).
5. **넘김 = 온보딩 스테이지 원리**: 장면은 페이지뷰 안에 있지만 무대 레이어는 페이지 이동을 상쇄한 뒤 레이어별 배율만큼만 움직인다(일러스트 120 / 칩 160 / BEST 240, fade 2.1·2.6). 옆 장면은 미리 빌드(`allowImplicitScrolling`)되고 중간 전엔 투명 — 스와이프 첫 프레임에 옆 그림이 불쑥 보이지 않는다. 투명한 옆 조각은 `IgnorePointer`로 탭을 가로채지 않게. 글(오버라인 accent 틱 + mono 번호 + 그룹 / 명조 `displayL` 이름(Hero `symptom-name-<id>`) / 한 줄 설명 / 먹색 원형 열기 버튼 52)은 24·48·72·96만큼 더 빨리 바깥으로 나가며 흐려진다.
