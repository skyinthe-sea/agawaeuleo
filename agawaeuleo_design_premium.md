...
> **⚠ 2026-09-24 — 시각 레이어는 DESIGN v3 `agawaeuleo_design_clay.md`("몽글 클레이")로 대체되었다.**
> 색·서체·모서리·음영·일러스트·컴포넌트 외형은 v3가 우선한다. 이 문서의 **모션 계약(§7.x 타이밍·
> ScrollReveal·패럴랙스·RevealGate·덱/노트 메커니즘·스플래시 비트)은 계속 유효**하다.
...
- **사용처**: 스플래시(로고 하단, stamp-in), 로그인 헤더 보조, 비밀번호 재설정 성공, 트래킹 저장 성공 모먼트, 설정>정보 그룹 상단, 약관 뷰어 말미.
...
### 4.8 `PaperBlobPainter` — 무대 배경 (`widgets/stage/paper_blob_painter.dart`, v2.3 신규)

온보딩 히어로 스테이지에서 태어나 홈 케어 덱·상세 케어 노트가 함께 쓰는 시그니처. 종이 오림 블롭(`paperStack` 아랫장 겹침) + 점선 궤도 + 행성 3개.
- 장면(페이지)마다 워시 색 1개, 윤곽 프리셋 3개를 번갈아 쓰고 **페이지 연속 값**으로 윤곽·색을 보간하며 조금 돈다(손가락을 따라 모핑). 그라데이션 없이 단색 면만.
- 좌표는 소비처의 설계 캔버스 단위(`FittedBox` 안 고정 캔버스). 중심·반경·궤도 배율·행성 시작각/속도/반지름은 소비처가 떠 있는 카드와 겹치지 않게 고른다.

### 4.9 `RevealGate` — 대기 장의 등장 보류 (`widgets/animated/scroll_reveal.dart`, v2.3 신규)

가로 페이저가 미리 빌드한 옆 장 안의 `ScrollReveal`은 세로 위치만 보고 화면 밖에서 재생을 끝내 버린다. 대기 장을 `RevealGate(open: ValueListenable<bool>)`로 감싸 **장이 화면에 들어오는 순간** 평소 규칙대로 판정·재생한다(관문이 없으면 늘 열림). 같은 개정에서 `ScrollReveal`의 스크롤 판정은 **레이아웃이 끝난 프레임 뒤**로 옮겼다 — 위치 변경 직후엔 좌표가 이전 프레임 값이라, 한 번에 크게 넘긴 짧은 드래그가 멈춘 자리의 위젯이 끝내 시작하지 못했다.

### 4.10 `InkIllustration.holdInkWeight` — 큰 히어로의 먹선 결 유지 (v2.3)

증상 일러스트(뷰박스 120)를 홈 덱(≈200dp)·상세 머리(≈112dp)처럼 크게 띄우면 먹선 3.0이 5dp를 넘어 뭉툭해진다. 켜면 배율 `s`(>1)에서 선 두께·도트 반지름·도트 간격이 `s^0.25`만큼만 커져 카드와 같은 결을 유지한다. 페인트 시점의 실제 크기로 계산하므로 Hero 비행 중에도 두께가 끊기지 않는다. 뷰박스 이하 크기는 영향 없음.
...
### 7.1 홈 (`features/home/`) — v2.3 "케어 덱" (2026-09-11, 증상 그리드 폐기)

> 발주자 요청: "단순히 그리드 펼쳐져 있는 것보다 애니메이션과 함께 넘기면서 보는 홈", 온보딩 스테이지의 느낌 그대로. 옛 v2 지시(2열 그리드·최근 칩)는 폐기.

1. **구성(위→아래)**: 인사(날짜 overline + 명조 인사 + 오늘의 응원 + 메뉴 — 현행) → 검색 알약 → **케어 덱**(그룹 탭 + 무대 + 글 + 레일). 덱이 남은 높이를 모두 쓰는 한 화면 구성, 작은 화면은 덱 최소 높이(540)를 지키며 세로 스크롤. 당겨서 새로고침(잉크 드롭) 유지. 글자 배율 상한 1.3.
2. **검색 알약**: 비어 있던 알약에 **타이핑 힌트** — 앞쪽 증상 4개 이름을 한글 IME 조합 순서(`core/utils/hangul_typing.dart`, 받침이 다음 음절로 넘어가는 순간까지 재현)로 쳤다 지우기를 **한 바퀴만** 돌고 정적 문구 "궁금한 증상을 검색해 보세요"로 쉰다(상시 반복 금지). reduce-motion은 처음부터 정적. Hero `home-search-bar` 유지.
3. **그룹 탭 + 카운터**: '아기 돌봄 25 / 엄마 돌봄 7'(body 700, 선택 ink900 + 20×3 먹 막대, 비선택 ink300, 개수 mono) + 우측 mono `01 / 25`(숫자만 위로 밀려 바뀜). 한쪽 그룹이 비면 그 탭은 없다. 그룹을 떠났다 돌아오면 보던 장에서 다시 연다.
4. **무대**(캔버스 360×300, contain): `PaperBlobPainter`(증상 `SymptomTone` 워시로 모핑, 중심 180,150·반경 112·궤도 1.17) + 일러스트 212(`holdInkWeight`, 블롭 중심에서 왼쪽 위로 18·12 비킴) + 떠 있는 **실데이터 조각**(e4): 좌상 칩 "추천 용품 N"(용품 없으면 "병원 신호 N") · 우하 **BEST PICK 카드**(1위 제품 제목 2줄, 폭 172). 조각 데이터는 상세와 같은 프로바이더를 공유(덱에서 본 증상은 상세 진입 시 이미 캐시).
   - **무대 조각은 전부 "누를 수 있는 것"**(v2.3.1, 2026-09-24 — 발주자 "추천용품 버튼 누르면 바로 그 페이지로"): 둘 다 끝에 `chevron_right` 18(ink300)을 세우고, 탭하면 상세를 그 장으로 바로 연다 — 칩은 자기가 세는 장(`추천 용품`→`?chapter=products` / `병원 신호`→`?chapter=emergency`), BEST 카드는 `?chapter=products`. 외부(쿠팡) 링크는 대가성 표시가 있는 상세에서만 열린다(§13.2) — BEST 카드의 옛 `arrow_outward`는 "앱을 벗어난다"는 오해를 줘 `chevron_right`로 교체. 접근성 라벨은 서로 겹치지 않게(칩 "{증상} 추천 용품 N개 보기" / 카드 "{증상} 추천 용품 1위 {제품}, 전체 보기").
5. **넘김 = 온보딩 스테이지 원리**: 장면은 페이지뷰 안에 있지만 무대 레이어는 페이지 이동을 상쇄한 뒤 레이어별 배율만큼만 움직인다(일러스트 120 / 칩 160 / BEST 240, fade 2.1·2.6). 옆 장면은 미리 빌드(`allowImplicitScrolling`)되고 중간 전엔 투명 — 스와이프 첫 프레임에 옆 그림이 불쑥 보이지 않는다. 투명한 옆 조각은 `IgnorePointer`로 탭을 가로채지 않게. 글(높이 **186** — 오버라인 accent 틱 + mono 번호 + 그룹 / 명조 `displayL` 이름(Hero `symptom-name-<id>`, 큰 글자 배율에서 먼저 양보하는 `Flexible`) / 한 줄 설명 / **'자세히 보기' 먹 캡슐**)은 24·48·72·96만큼 더 빨리 바깥으로 나가며 흐려진다.
   - **열기 어피던스 = 글자 있는 전폭 먹 캡슐**(v2.3.1 — 발주자 "처음 쓰는 사람은 그림을 눌러야 하는 줄 모른다"): 높이 52·`brFull`·`ink900` + `e2`, 좌 `label`/`paperRaised` "자세히 보기" + 우 `arrow_forward` 20, 좌우 x20 패딩, 누름 .98. 옛 먹색 **원형 화살표 52**는 폐기 — 무대·글 어디를 눌러도 같은 곳으로 가지만 "여기를 누르세요"는 이 캡슐이 진다. 글 영역 186은 배율 1.3에서도 넘치지 않는 값이고, 그만큼 덱 최소 높이가 470→540이 됐다.
6. **마이크로 인터랙션**(스크롤이 **멈춘** 뒤): 일러스트 톡 튀기(720ms, 1.055배 + 감쇠 흔들림) · 1위 배지 도장(1.7→1). 세션 첫 진입 1회 등장(배경 → 일러스트 → 칩 → 카드 → 글, 900ms) 뒤 **넘기기 힌트**(첫 장이 화면 폭 9%만큼 밀렸다 돌아옴, 1.1s — 손을 대거나 그룹을 바꾸면 즉시 중단). 조각 호흡 ±3dp 상시. reduce-motion이면 전부 정지(무대 레이어의 손가락 추종은 직접 조작이라 유지).
7. **증상 레일**(높이 92): 동그란 종이 썸네일 52 + 이름. 덱과 양방향 — 넘기면 레일이 따라 흘러 현재 증상을 **왼쪽 두 번째 자리**로 데려오고(앞 증상 하나를 남겨 "어디서 왔고 다음이 무엇인지"), 링(line→ink900, 1→2dp)·크기(.84→1)·라벨 색이 페이지 연속 값을 따른다. 썸네일 탭 → 3장 이내면 넘기고, 멀면 바로 간다. 사용자가 레일을 직접 굴리는 동안은 따라가기 중단.
8. Scaffold body `PaperBackground`. `SymptomCard`(옛 그리드 카드)는 컴포넌트·테스트만 보존, 최근 본 증상 칩은 제거(최근 기록 저장은 유지).
...
> **v2.3 "케어 노트"(2026-09-11) — 긴 한 장 스크롤 폐기.** 아래 1~2·4는 옛 한 장 구성의 지시로 머리·요약 표면 문법만 계승하고, 5(면책)·6(제품 보드)·7(응급 카드)은 그대로 유효하다.
>
> - **메커니즘(홈 덱과 일부러 다르게)**: 장(요약 → 병원 신호 → 돌보는 법 → 추천 용품 — 데이터 있는 장만)을 **종이를 한 장씩 겹쳐 올리듯** 넘긴다. 들어오는 장은 손가락과 1:1로 위에 덮이며(앞 가장자리 그림자, `e3` 색 파생·blur 36), 덮이는 장은 이동의 22%만 따라가며 제자리 근처에서 축소(0.95)·그늘로 가라앉는다. 되돌아가면 거꾸로 걷힌다. **장 위젯 트리 구조는 늘 같게**(분기마다 모양이 바뀌면 장의 스크롤 위치·등장 상태가 초기화됨).
> - **머리(장 위에 떠 있음)**: 위 막대 56(동그란 종이 버튼 44 — 뒤로·즐겨찾기 Sparkle) + 히어로 줄 148(좌: accent 틱 "케어 노트 · 그룹" / 명조 `display` 이름(Hero) / 한 줄 설명, 우: 무대 140 — `PaperBlobPainter` 워시가 장마다 증상 톤→coral→sage→amber로 모핑 + 일러스트 112(Hero, 넘김 중간에 한 번 기울었다 섬)) + **장 탭** 52(같은 폭 칸, 먹 막대 24×3이 페이지 연속 값으로 미끄러짐, 글자색 보간, 추천 용품 개수 amber mono). 현재 장을 스크롤하면 히어로 줄이 접혀 올라가고(최대 148) 위 막대에 명조 이름이 떠오르며 아래 그림자 e2가 선다. 장 사이 넘기는 동안은 두 장의 접힘을 페이지 값으로 보간하고, 넘기기 직전 다른 장의 접힘을 현재 장에 맞춘다(머리 널뛰기 방지). 히어로 줄은 포인터 통과(아래는 장의 빈 윗여백), 위 막대·탭은 아래로 탭이 새지 않게 막는다. 접힌 상태에선 `HeroMode` off(보이지 않는 자리에서 날아가지 않게).
> - **장 구성**: ① 요약 — hero 카드 "한눈에 보기"(본문 `keepAll`) + "이렇게도 불러요" 별칭 알약 + 붓결 디바이더 + `SectionHeader(CONTENTS / 이 노트에 담긴 것)` 목차(장 번호 mono · 아이콘 톤 · 메타 "꼭 알아 둘 신호 4가지" / "집에서 돌보는 가이드 9편" / "3개 · 1위 {제품}", 탭 → 그 장). ② 병원 신호 — `EmergencyCard`(장이 화면에 들어올 때 흔들림·펄스 1회). ③ 돌보는 법 — `SectionHeader(집에서 돌보는 법, N편)` + 아코디언(등장은 화면 진입 시 `ScrollReveal`, 계단 60ms·상한 2단) + 참고 자료. ④ 추천 용품 — `ProductSection`(보드·대가성 배지 그대로). 요약 외 장 끝엔 **NEXT 카드**(mono `dataL` 번호 + 장 이름 + 먹색 원 화살표) → 다음 장. **모든 장 끝에 §13.3 면책**(장 콘텐츠는 한 덩어리로 빌드해 늘 트리에 있음).
> - **등장**: 옆 장은 미리 빌드하되 `RevealGate`로 장이 화면에 들어오는 순간 재생. `?chapter=products` 같은 딥링크는 장 구성이 확정될 때까지 노트를 가렸다가(최대 1.2s 안전망) 그 장으로 넘겨 연다.
> - 로딩·에러·없음 상태는 기존 앱바 + 스켈레톤/`ErrorState`/`EmptyState`.
...
6. **제품 섹션** (v2.1 개정 — "에디토리얼 랭킹 보드"): 헤더 → `SectionHeader`(trailing = "N개" data체 알약). 대가성 배지는 `amberWash` 토큰 + `paperRaised` 원형 아이콘, **문구 원문 불변**. 리스트는 균일 카드 나열이 아니라 **1위 히어로 + 2위 이하 랭킹 행**으로 위계를 나눈다.
   ...
   - **모션**(전부 reduce-motion 대체) — *한 번만 보이는 등장은 확실하게, 상시 반복은 은은하게*가 원칙:
     - **등장**(`ScrollReveal`, slow×1.1 = 440ms): 카드 상단이 화면 아래 **10%** 지점까지 올라오면 재생. 상승 24 + scale .94(스프링, 끝에서 살짝 넘겼다 앉음) + 페이드(불투명도는 0.45, 이동은 0.6 지점에서 종료 — 먼저 뜨고 마지막에 앉는 순서라야 빠릿하게 읽힌다). 헤더 → 대가성 배지 → 카드 순으로 60ms 계단, **상한 2단(120ms)**.
       - **타이밍 계약**(발주자 피드백 2회 반영): 트리거가 너무 작으면 화면 밖에서 끝나 체감이 0이고, 너무 크거나 길이/계단이 길면 "목록이 늦게 뜬다"가 된다. 지연은 뷰포트 진입 **후**부터 세므로 상한 상향은 특히 위험. 현재 값이 두 실패 사이의 중간점이며, 올릴 때는 실기기에서 느린 스크롤로 재확인할 것.
     - **내부 시퀀싱**(`ScrollRevealScope` + `RevealMotion`): 카드가 앉는 동안 순위 배지가 1.7배에서 도장 찍히듯(0.3~0.75, 스프링) 앉고, 사진은 1.06배에서 제자리로 줌-세틀(~0.8), CTA 알약은 16dp 아래에서 밀려 올라온다(0.45~0.9). 스탬프가 잘리지 않도록 썸네일 `Stack`은 `Clip.none`.
...
> **2026-09-11 숨김**: 제품 추천 집중으로 기록 탭·브랜치를 주석 처리했다(화면·데이터 코드 보존). 아래 지시는 복원 시의 계약으로 남긴다.
...
6. **온보딩** (v2.2 개정 — "히어로 스테이지", 2026-09-11. 아이콘-in-프레임·스텝 라벨·마지막 장 낙관은 폐기):
   - **서사**: 우는 아기(증상 검색) → 케어 가방(맞춤 용품) → 구름 위 잠(**병원 신호** — 2026-09-11 기록 숨김으로 "육아 기록"에서 교체. 차분히 확인하고 안심하는 밤). 일러스트 3종은 증상 일러스트와 같은 엔진·그림체(`InkIllustration`, 뷰박스 **240** — 선 수치는 그대로 두고 뷰박스만 키워 히어로에서도 먹선 굵기를 카드와 맞춤). **수정은 `tool/illustrations/generate_onboarding_illustrations.py --dart`에서만**(생성 파일 직접 편집 금지).
   - **구성(위→아래)**: 좌측 인디케이터(활성 알약 `ink900`, 폭·색이 페이지 **연속 값**을 따라감) + 우측 "건너뛰기"(마지막 장은 CTA와 중복이라 숨김) → 스테이지(공유 배경 + 일러스트 + 떠 있는 실제 UI 조각 2개, e4) → 오버라인(accent 틱 + mono 번호 + 한글 라벨 — 한글엔 overline 자간 +1.2 금지) → 제목 `displayL` **줄을 카피에서 직접 나눔**(좁으면 줄마다 scaleDown, 재줄바꿈 없음) → 본문 `bodyL` 어절 단위 줄바꿈(`keepAll`) → CTA.
   - **스테이지는 페이지뷰 밖 고정 무대**: 세 장면을 항상 올려 두고 페이지 값에 1:1로 레이어별 이동·페이드(일러스트 110 / 뒤 카드 150 / 앞 카드 230). 옛 "느린 패럴랙스"는 옆 페이지 그림을 화면 안으로 끌어와 스와이프 첫 프레임에 불쑥 보이는 버그의 원인이었으므로 **금지**. 일러스트는 중간 지점 전에 사라져(fade 2.1) 선화 두 장이 반투명으로 겹쳐 탁해지지 않게 한다. 공유 배경은 종이 오림 블롭(`PaperBlobPainter`, `paperStack` 아랫장) — 장면 워시(accent→amber→coral)와 윤곽이 모핑, 점선 궤도·행성 3개(카드와 겹치지 않는 각도).
   - **글**: 페이지와 함께 움직이되 오버라인 24 / 제목 48 / 본문 72만큼 더 빨리 나감(항상 화면 바깥 방향) + 페이드.
   - **마이크로 인터랙션**(스크롤이 **멈춘** 뒤 재생 — `onPageChanged`는 0.5 지점이라 쓰지 않음): 검색어 한글 IME 자모 타이핑(`hangulTypingSteps`) + 캐럿, 1위 배지 스탬프, ③ "배앓이 병원 신호 4" 칩의 숨쉬는 인주 점 + 응급 카드 미리보기(신호 두 줄이 차례로 내려앉고 "가까운 병원 찾기" 버튼이 톡 튀어 오름). ③ 블롭 워시는 coralWash. 첫 진입 1회 등장(배경→일러스트→카드, 900ms), 카드 호흡 ±3dp 상시. reduce-motion이면 전부 완료 상태 정지.
7. 재설정 성공·권한 프라이밍의 원 → `InkHaloIcon`(각 96/120, animate).

### 7.7 프로필·설정 (`features/profile/`, `features/settings/`)

> **2026-09-11 기록 숨김 반영(주석 — 복원 가능)**: 내 정보 메뉴의 '아기 프로필'(기록을 아기별로 나누는 메뉴)과 설정 알림의 '수유 리마인더'를 숨겼다. 게스트 헤더 안내는 "즐겨찾기는 이 기기에 안전하게 저장돼요", 알림 권한 안내·프라이밍 화면은 수유·기록 약속을 뺀 문구(원문은 코드 주석에 보존).
