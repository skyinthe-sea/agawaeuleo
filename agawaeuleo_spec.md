...
> **⚠ v1.3.1 재개정 (2026-09-24)**: 시각 레이어는 다시 **`agawaeuleo_design_clay.md`(DESIGN v3 "몽글 클레이")**로 개정되었다 — 색·서체·일러스트·컴포넌트 외형은 v3가 우선, 모션은 v2 계약 유지.
>
> **⚠ v1.2 개정 (2026-07-10)**: 본 장(§9)·§10·§11의 **시각 레이어는 `agawaeuleo_design_premium.md`(DESIGN v2 "묵직한 페이퍼잉크")로 개정**되었다. 토큰 값·컴포넌트·운용 규칙이 충돌하면 해당 문서가 우선한다. 아래 표는 v1.1 원본 기록으로 보존.
...
- **동작**: 행 탭 → 해당 화면(push). 아바타 탭 → 계정 관리(게스트면 계정 연결 유도, 연결됨이면 설정 계정 그룹).
...
- **동작**:
  ...
  - 로그아웃 탭 → 확인 다이얼로그 → 연결 계정 세션 종료 → **익명(게스트) 세션 복귀 → 홈**(§3.3 게스트 우선 — 인증 화면 강제 복귀 금지).
...
### 16.4 변경 이력
- **v1.3.1 (2026-09-24) — 디자인 v3 "몽글 클레이"**(기능·데이터·문구 변경 없음, 시각 레이어만)
  1. **시각 레이어 재개정**: 부속 계약 `agawaeuleo_design_clay.md`(DESIGN v3) 신설 — v2(`agawaeuleo_design_premium.md`)의 색·서체·모서리·음영·일러스트·컴포넌트 외형을 대체한다(모션 계약은 v2 유지). 초보맘 대상의 여성스럽고 아기자기한 무드: 딸기우유 크림 바탕 + 코코아 텍스트 + 로즈 액센트(`accentFill` 신설 — 채운 알약 면 전용, 큰 글씨 규칙) + 파스텔 카테고리 워시 + 엄마 돌봄 라일락(`lilac`/`lilacWash` 신설), 제목 서체 명조 → **주아체(OFL)**, 모서리 한 단계 상향, 장밋빛 부드러운 음영, 클레이 광택 그라데이션만 허용.
  2. **일러스트 전면 교체**: 먹선 벡터(32종 + 온보딩 3) → **3D 클레이 렌더 WebP**(증상 32 + 온보딩 3 + 스플래시 얼굴 2 + 빈 상태·내 정보 장면 4). 생성기 `tool/illustrations/clay/`(numpy 높이맵 점토 렌더러 — 에셋 직접 편집 금지).
  3. **앱 아이콘·네이티브 스플래시**: 인주 도장 → 딸기 핑크 바탕 + 클레이 우는 아가 얼굴(같은 엔진). 런치 배경 크림 `#FFF6EF`(다크 `#221A19`).
- **v1.3 (2026-07-12)**
...
*문서 끝. 이 설계서는 v1.3.1이며(시각 레이어는 `agawaeuleo_design_clay.md`, 모션은 `agawaeuleo_design_premium.md` 참조), 콘텐츠(증상/제품) 및 정책(파트너스 승인 기준·스토어 가이드라인)은 시점에 따라 변할 수 있으니 착수 시 최신 상태를 확인할 것.*
