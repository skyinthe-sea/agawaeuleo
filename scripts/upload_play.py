...
# 내부 테스터가 마지막으로 받은 빌드 이후의 변경 요약(1.1.0+8 — 홈 진입 어피던스 개선).
# 릴리스마다 갱신하거나 `--notes`로 덮어쓸 것 — 지난 빌드 문구가 그대로 올라가면
# 테스터가 무엇이 바뀌었는지 알 수 없다.
DEFAULT_NOTES = (
    "홈 화면을 처음 쓰는 분도 헤매지 않도록 다듬었어요. "
    "증상 이름 아래에 '자세히 보기' 버튼이 생겨 어디를 눌러야 하는지 한눈에 보이고, "
    "그림 위의 '추천 용품 N' 칩을 누르면 그 증상의 추천 용품 목록이 바로 열립니다. "
    "BEST PICK 카드도 같은 자리로 이어져요."
)
